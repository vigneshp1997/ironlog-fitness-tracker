...
function ExerciseSelector({ exercises, onSelect, selectedIds }) {
  const items = [];
  for (let i = 0; i < exercises.length; i++) {
    ...
    items.push(
      <button
        key={ex.id}
        onClick={() => onSelect(ex)}
        className={`w-full p-3 rounded-lg border transition-all duration-200 text-left flex items-center justify-between group ${isSelected ? "bg-primary/20 border-primary" : "bg-secondary hover:bg-secondary/80 border-transparent hover:border-primary/50"}`}
...
function TemplateCard({ template, onEdit, onDelete }) {
  return (
    <Card className="border-border/40 bg-card card-hover" data-testid={`template-card-${template.id}`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" onClick={() => onEdit(template)} data-testid={`edit-template-${template.id}`}>
              <Edit className="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => onDelete(template)} className="text-destructive hover:text-destructive" data-testid={`delete-template-${template.id}`}>
      ...
      <CardContent>
        ...
        <div className="flex flex-wrap gap-2">
          {template.exercises.map((ex, i) => {
            const isCardio = ex.category === "cardio";
            return (
              <Badge key={i} variant="secondary" className={`badge-${isCardio ? "cardio" : "chest"}`}>
                {ex.exercise_name}
              </Badge>
            );
          })}
...
export default function Templates() {
  ...
  const selectedIds = selectedExercises.map(e => e.exercise_id);
      ...
      {templates.length === 0 ? (
        <Card className="border-border/40 bg-card border-dashed">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">Create workout templates like "Push Day" or "Leg Day"</p>
            <Button onClick={openCreateModal} variant="outline" data-testid="create-first-template-btn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map(t => <TemplateCard key={t.id} template={t} onEdit={openEditModal} onDelete={setDeleteTemplate} />)}
        </div>
      )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Push Day, Leg Day" className="mt-2 bg-secondary border-border" data-testid="template-name-input" />
              </div>
              <div>
            ...
            <div>
              <Label className="text-sm uppercase tracking-widest text-muted-foreground">Selected Exercises ({selectedExercises.length})</Label>
              {selectedExercises.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2 p-3 bg-secondary rounded-lg">
                  {selectedExercises.map((ex, i) => (
                    <Badge key={i} variant="secondary" className="flex items-center gap-1">
                      {ex.exercise_name}
                      <button onClick={() => handleExerciseToggle({ id: ex.exercise_id })} className="ml-1 hover:text-destructive">
                        <X className="w-3 h-3" />
                      </button>
                    </Badge>
                  ))}
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-3">
                ...
                <Select value={muscleFilter} onValueChange={setMuscleFilter}>
                  <SelectTrigger className="w-full sm:w-[180px] bg-secondary border-border" data-testid="template-muscle-filter">
              ...
              <ScrollArea className="h-[250px] pr-4 border border-border rounded-lg p-2">
                <div className="space-y-2">
                  <ExerciseSelector exercises={filteredExercises} onSelect={handleExerciseToggle} selectedIds={selectedIds} />
